...
/**\
 * Doylu V1 (3 fichiers)\
 * - OFFERS + SOURCES stock\'e9s ici (temporaire)\
...
\
  OFFERS = [\
    // --- ORANGE (USSD OFFLINE #1234#) \'97 d'apr\'e8s tes captures ---\
    \{\
      ...
      validity_days: null,\
      ussd_code: "#1234#",\
      activation_path: "#1234# > Je choisis mon pass > 1 (300Mo \'e0 200F)",\
      status: "active",\
      confidence_score: 0,\
      is_verified: false,\
      last_seen_at: t,\
    ...
    \{\
      ...
      validity_days: null,\
      ussd_code: "#1234#",\
      activation_path: "#1234# > Je choisis mon pass > 2 (1,5Go \'e0 500F)",\
      status: "active",\
      confidence_score: 0,\
      is_verified: false,\
      last_seen_at: t,\
    ...
    \{\
      ...
      validity_days: null,\
      ussd_code: "#1234#",\
      activation_path: "#1234# > Je choisis mon pass > 3 (5Go \'e0 1000F)",\
      status: "active",\
      confidence_score: 0,\
      is_verified: false,\
      last_seen_at: t,\
    ...
    \{\
      ...
      validity_days: 1,\
      ussd_code: "#1234#",\
      activation_path: "#1234# > (menu data) > 3,5Go 24H > payer (OM ou cr\'e9dit)",\
      status: "active",\
      confidence_score: 0,\
      is_verified: false,\
      last_seen_at: t,\
...
\
  OFFER_SOURCES = [\
    \{\
      ...
      platform: "ussd",\
      source_url: null,\
      raw_text: "Je choisis mon pass: 1:300Mo \'e0 200F; 2:1,5Go \'e0 500F; 3:5Go \'e0 1000F",\
      media_url: null,\
      received_at: t,\
    ...
    \{\
      ...
      platform: "ussd",\
      source_url: null,\
      raw_text: "3,5Go valable 24H \'e0 700F. 1: Orange Money 2: cr\'e9dit",\
      media_url: null,\
      received_at: t,\
      submitted_by: "frere_ussd",\
      is_official: true,\
...
function metaLine(o) \{\
  const parts = [];\
  if (o.data_mb) parts.push(`\uc0\u55357 \u56561  $\{(Number(o.data_mb) / 1024).toFixed(Number(o.data_mb) % 1024 === 0 ? 0 : 1)\} Go`);\
  if (o.minutes) parts.push(`\uc0\u55357 \u56542  $\{o.minutes\} min`);\
  parts.push(`\uc0\u9201  $\{validityLabel(o)\}`);\
  return parts.join(" \'95 ");\
\}\
function oneBadge(offer) \{\
  if (offer.is_verified) return \{ cls: "verified", label: "V\'e9rifi\'e9" \};\
  if (offer.is_sponsored) return \{ cls: "sponsor", label: "Sponsoris\'e9" \};\
  const \{ topSources \} = computeConfidenceForOffer(offer);\
  const top = topSources[0];\
  if (top && top.source_type === "sms") return \{ cls: "official", label: "Confirm\'e9 SMS" \};\
  if (top && (top.source_type === "website" || (top.source_type === "social" && top.is_official))) return \{ cls: "official", label: "Officiel" \};\
  return \{ cls: "neutral", label: "\'c0 confirmer" \};\
\}\
\
function toggleLoader(on, text = "Recherche en cours\'85") \{\
...
\
function setLastUpdate() \{ $("lastUpdate").textContent = `aujourd\'92hui $\{nowHHmm()\}`; \}\
...
\
  toggleLoader(true, "Recherche en cours\'85");\
...
\
  ...
  revealBtn.textContent = "\uc0\u9989  Code affich\'e9";\
...
\
  ...
  renderPopular();\
\
  showToast("Code copi\'e9. Compose-le maintenant.");\
  setTimeout(() => \{\
...
function shareWhatsApp(title, price, ussd) \{\
  const msg = `Bon plan Doylu \uc0\u55356 \u56824 \u55356 \u56819 \\n$\{title\}\\nPrix: $\{price\} FCFA\\nCode: $\{ussd\}\\n`;\
...
function openWhatsAppSupport() \{\
  window.open(`https://wa.me/?text=$\{encodeURIComponent("Salut Doylu, j\'92ai une question / un signalement.")\}`, "_blank");\
\}\
\
  ...
  if (results.length === 0) \{\
    container.innerHTML = `\
      <div class="offer-card">\
        ...
        <div class="meta">Essaie un autre budget ou change usage/validit\'e9.</div>\
      </div>\
    `;\
  \} else \{\
    container.innerHTML = results.map((o) => \{\
      const badge = oneBadge(o);\
      const title = `$\{o.operator\} \'97 $\{o.name\}`;\
...
\
          <button class="btn cta2" id="reveal_$\{o.offer_id\}" type="button">\uc0\u55357 \u56385 \u65039  Afficher le code</button>\
          <div class="ussd-box" id="ussd_$\{o.offer_id\}">$\{escapeHtml(ussd)\}</div>\
\
          <div class="row-actions">\
            <button class="btn gray" id="copy_$\{o.offer_id\}" type="button" style="display:none">\uc0\u55357 \u56523  Copier</button>\
            <button class="btn whatsapp" id="share_$\{o.offer_id\}" type="button">\uc0\u55357 \u57314  Partager WhatsApp</button>\
          </div>\
\
          <div class="inline-nudge" id="nudge_$\{o.offer_id\}">\
            Ne rate plus les meilleures offres.\
            <span class="meta2">\uc0\u55357 \u57314  Gratuit \'95 1 message/jour max.</span>\
  ...
  if (!promos.length) \{\
    container.innerHTML = `\
      <div class="offer-card">\
        ...
        <div class="meta">\'c7a se remplit d\'e8s qu\'92on ajoute des SMS promos.</div>\
      </div>\
    `;\
    return;\
  \}\
...
\
  if (!ranked.length || ranked.every((x) => x._p === 0)) \{\
    el.textContent = "Pas encore de donn\'e9es ce mois-ci. Clique un budget pour d\'e9marrer.";\
    return;\
  \}\
\
  el.innerHTML = ranked.map((o, i) => `\
    <div style="display:flex;justify-content:space-between;gap:10px;padding:10px 0;border-top:$\{i === 0 ? "0" : "1px solid #f1f5f9"\}">\
      <div class="bold">$\{escapeHtml(o.operator)\} \'97 $\{escapeHtml(o.name)\}</div>\
...
function openHowVerify() \{\
  openModal("Comment on v\'e9rifie ?", `\
    <ul>\
      <li>On collecte des offres re\'e7ues par SMS et vues dans les menus USSD.</li>\
      <li>On v\'e9rifie la coh\'e9rence (prix, volume, validit\'e9) et on retire les expir\'e9es.</li>\
      <li>Le badge \'93V\'e9rifi\'e9\'94 appara\'eet quand il y a plusieurs preuves r\'e9centes.</li>\
    </ul>\
  `);\
\}\
\
...
\
    try \{\
      ...
      closeModal();\
      showToast("Import coll\'e9 OK \uc0\u9989 ");\
    \} catch \{\
...
function openAddOfferModal() \{\
  openModal("Ajouter une offre", `\
    <div class="form-grid">\
      <div>\
        <div class="small-muted">Op\'e9rateur</div>\
...
\
      <div>\
        <div class="small-muted">Nom de l\'92offre</div>\
...
\
      <div>\
        <div class="small-muted">Validit\'e9 (jours)</div>\
        <input id="f_validity_days" type="number" placeholder="Ex: 7" />\
      </div>\
      <div>\
        <div class="small-muted">Validit\'e9 type</div>\
...
\
      ...
      <div>\
        <div class="small-muted">Sponsoris\'e9</div>\
...
\
    <div class="small-muted" style="margin-top:10px">Chemin d\'92activation (optionnel)</div>\
...
\
    ...
    renderPopular();\
    if (selectedBudget) renderResults();\
    showToast("Offre ajout\'e9e \uc0\u9989 ");\
  \});\
\}\
\
function openAddSourceModal() \{\
  const offerOptions = OFFERS.map((o) => `<option value="$\{escapeHtml(o.offer_id)\}">$\{escapeHtml(o.operator)\} \'97 $\{escapeHtml(o.name)\}</option>`).join("");\
\
...
\
    ...
    renderPopular();\
    if (selectedBudget) renderResults();\
    showToast("Source ajout\'e9e \uc0\u9989 ");\
  \});\
\}\
\
/* -----------------------\
...
function renderAdmin() \{\
  ...
  list.innerHTML = OFFERS.map((o) => `\
    <div class="admin-item">\
      <div class="bold">#$\{escapeHtml(o.offer_id)\} \'95 $\{escapeHtml(o.operator)\} \'95 $\{escapeHtml(o.name)\}</div>\
      <div class="small-muted">conf: $\{o.confidence_score\}/100 \'95 status: $\{escapeHtml(o.status)\} \'95 pop: $\{o.popularity_score\}</div>\
      <div class="hr"></div>\
\
...
\
      <div class="admin-row">\
        ...
        <label><input data-offer="$\{o.offer_id\}" data-field="is_sponsored" type="checkbox" $\{o.is_sponsored ? "checked" : ""\}/> sponsoris\'e9</label>\
...
function adminImport(file) \{\
  ...
  reader.onload = () => \{\
    try \{\
      ...
      applyImportedBundle(bundle);\
      showToast("Import fichier OK \uc0\u9989 ");\
    \} catch \{\
...
\
  ...
  $("adminRecomputeBtn").addEventListener("click", () => \{ recomputeAll(); renderAdmin(); renderPopular(); if (selectedBudget) renderResults(); showToast("Recalcul OK \uc0\u9989 "); \});\
